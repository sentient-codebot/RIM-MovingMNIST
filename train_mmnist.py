...
def dist_run(rank, world_size, args, columns):
    ...
    model, optimizer, scheduler, loss_fn, start_epoch, train_batch_idx, best_mse = setup_model_dist(args, rank)
    
    for epoch in range(start_epoch, args.epochs+1):
        train_loader.sampler.set_epoch(epoch)
        train_batch_idx, train_loss_, train_recon_loss_, train_pred_loss_ = train_dist(rank, model, train_loader, optimizer, start_epoch, train_batch_idx, args, loss_fn)
        train_loss = [None for _ in range(world_size)]
        train_recon_loss = [None for _ in range(world_size)]
        train_pred_loss = [None for _ in range(world_size)]
        
        dist.all_gather_object(train_loss, train_loss_)
            ...
            if args.test_frequency > 0 and epoch % args.test_frequency == 0 or epoch <= 15:
                ...
                wandb.log({
                    ...
                    'Stats': {
                        ...
                        'Past Slot Init Scale': 0. if not args.use_past_slots else torch.sigmoid(model.module.slot_attention.manual_init_scale_digit).detach()},
                }, step=epoch)
