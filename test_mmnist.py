...
def test_dist(model, test_loader, args, loss_fn, rollout=True, epoch=0, log_columns=None):
    ...
    for batch_idx, data in enumerate(tqdm(test_loader) if __name__ == "__main__" else test_loader): # tqdm doesn't work here?
        ...
        for frame in range(data.shape[1]-1):
            ...
            if do_logging:
                blocked_prediction[:, 0, frame+1, :, :, :] = preds # dim == 6
                blocked_prediction[:, 1:, frame+1, :, :, :] = model.module.hidden_features['individual_output']
                if recons is not None:
                    reconstruction.append(recons) # [BS, C, H, W]
                    if not args.decode_hidden:
                        individual_recons.append(model.module.hidden_features['individual_recons']) # [BS, K, C, H, W]
                ...
                for sample_idx in range(data.shape[0]):
                    ...
                    if 'SEP' in args.decoder_type:
                        table_row['individual_prediction'] = wandb.Image(make_grid(model.module.hidden_features['individual_output'][sample_idx]*255, pad_value=255)), # N K C H W -> K C H W -> C *H **W
                    if args.core == 'RIM' or args.core == 'SCOFF':
                        table_row['input attention probs'] = wandb.Image(
                            plot_heatmap(
                                model.module.rnn_model.hidden_features['input_attention_probs'][sample_idx],  # [num_hidden, num_inputs]
                                x_label = 'Slots' if args.use_slot_attention else 'Features',
                                y_label = 'RIMs' if args.core == 'RIM' else 'OFs',
                                vmin=0.,
                                vmax=1.,
                    ...
                    if args.core == 'SCOFF':
                        rule_attn_probs = model.module.rnn_model.hidden_features['rule_attn_probs'][sample_idx] # [num_hidden, num_rules]
                        for of_idx in range(args.num_hidden):
                            table_row.update({ 
                                f'rule_OF_{of_idx}': rule_attn_probs[of_idx].tolist(), # list of length num_rules, rule distribution
                            })
                ...
                if args.core == 'RIM' or args.core == 'SCOFF':
                # wandb/tb logging for concatenated image
                    dec_util.append(model.module.rnn_model.hidden_features.get("decoder_utilization", torch.zeros(1, 1)))
                most_used_units.append(0)
                if args.core == 'RIM':
                    rim_actv.append(model.module.rnn_model.hidden_features['input_attention_probs']) # shape (batchsize, num_units, 1) -> (BS, NU, T)
                    input_attn_probs.append(model.module.rnn_model.hidden_features['input_attention_probs'].unsqueeze(1)) # Shape: [N, 1, num_hidden, num_inputs]
                    rim_actv_mask.append(model.module.rnn_model.hidden_features["input_attention_mask"])
                    if 'rule_attn_probs' in model.module.rnn_model.hidden_features:
                        rule_attn_probs_list.append(model.module.rnn_model.hidden_features['rule_attn_probs'].unsqueeze(1)) # NOTE [N, 1, num_hidden, num_rules]
                elif args.core == 'SCOFF':
                    rule_attn_argmax.append(model.module.rnn_model.hidden_features['rule_attn_argmax']) # [N, num_hidden] -> [N, num_hidden, T]
                    rule_attn_probs_list.append(model.module.rnn_model.hidden_features['rule_attn_probs'].unsqueeze(1)) # NOTE [N, 1, num_hidden, num_rules]
                    if 'input_attention_probs' in model.module.rnn_model.hidden_features:
                        input_attn_probs.append(model.module.rnn_model.hidden_features['input_attention_probs'].unsqueeze(1)) # Shape: [N, 1, num_hidden, num_inputs]
